...
void          ft_setarg(t_ws *prm)
{
    ...
    err |= clSetKernelArg(prm->kernel, 3, sizeof(int), &prm->dec_x);
    err |= clSetKernelArg(prm->kernel, 4, sizeof(int), &prm->dec_y);
    ...
    err |= clSetKernelArg(prm->kernel, 8, sizeof(int), &prm->it);
    if (err != CL_SUCCESS)
    {
...
int         ft_calc_fractal(t_ws *prm)
{
    ...
    err = clEnqueueNDRangeKernel(prm->commands, prm->kernel, 1, NULL, &prm->global, NULL, 0, NULL, NULL);
    if (err)
    {
